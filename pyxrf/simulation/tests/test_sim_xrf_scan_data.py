from pyxrf.simulation.sim_xrf_scan_data import (
    ...
    create_xrf_map_data, gen_hdf5_qa_dataset, gen_hdf5_qa_dataset_preset_1)
import pytest
import numpy as np
...
def test_gen_xrf_spectrum_2():
    ...
    spectrum, xx = gen_xrf_spectrum(element_groups, incident_energy=12.0)
    npt.assert_array_equal(spectrum, spectrum_expected,
...
def test_create_hdf5_xrf_map_const(tmp_path):
    ...
    fln_path = create_hdf5_xrf_map_const(scan_id=scan_id, wd=wd,
                                         element_line_groups=element_groups,
                                         nx=100, ny=50,
                                         background_area=0)
    ...
    fln_suffix = "hello123"
    fln_path = create_hdf5_xrf_map_const(scan_id=scan_id, wd=wd, fln_suffix=fln_suffix,
                                         element_line_groups=element_groups,
                                         nx=100, ny=50,
                                         background_area=0)
